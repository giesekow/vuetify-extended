...
const field_table_widgets_1 = require("./widgets/field-table-widgets");
const field_pagination_widget_1 = require("./widgets/field-pagination-widget");
const field_pagination_state_1 = require("./widgets/field-pagination-state");
const field_autocomplete_table_state_1 = require("./widgets/field-autocomplete-table-state");
require("@vuepic/vue-datepicker/dist/main.css");
const dialogs_1 = require("./dialogs");
const dialog_lifecycle_1 = require("./dialog-lifecycle");
const nested_property_1 = __importDefault(require("nested-property"));
const katex_1 = __importDefault(require("katex"));
require("katex/dist/katex.min.css");
const runtime_1 = require("./runtime");
const decimal_1 = require("../misc/decimal");
...
class Field extends base_1.UIBase {
    constructor(params, options) {
        ...
        this.htmlValidationResult = this.$makeRef();
        this.htmlValidationVersion = 0;
        this.handledModelSyncPending = false;
        this.handledModelSyncVersion = 0;
        this.initialized = false;
        this.initializationVersion = 0;
        this.popupOwners = [];
        this.popupRequest = this.$makeRef();
        this.collectionPopupReady = this.$makeRef(false);
        this.collectionPopupGeneration = 0;
    ...
    get $value() {
        ...
        if (this.params.value.type === 'decimal') {
            if (Array.isArray(val)) {
                return val.map((item) => { var _a; return (_a = item === null || item === void 0 ? void 0 : item.$numberDecimal) !== null && _a !== void 0 ? _a : item; });
            }
    ...
    setModelValueFromMaster(value) {
        this.modelValue.value = value;
        this.markCurrentModelSyncHandled();
    }
    setModelValueAndSync(value, origin = 'user') {
        const previousValue = this.modelValue.value;
        this.modelValue.value = value;
        this.markCurrentModelSyncHandled();
        this.valueChanged(value, origin, previousValue);
    }
    normalizePaginationValue(value) {
        return (0, field_pagination_state_1.normalizeFieldPaginationValue)(value, {
            page: this.params.value.page,
            limit: Number(this.params.value.itemsPerPage || 10),
            total: this.params.value.totalItems,
        });
    }
    paginationEvent(value, previousValue, reason) {
        return (0, field_pagination_state_1.createFieldPaginationEvent)(value, previousValue, reason);
    }
    get $pagination() {
        return this.normalizePaginationValue(this.modelValue.value);
    }
    get $paginationItemsPerPageOptions() {
    ...
    setPagination(value, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const previousValue = this.$pagination;
            const reason = options.reason || 'programmatic';
            const nextValue = (0, field_pagination_state_1.updateFieldPaginationValue)(previousValue, value, reason);
            if (nextValue.page === previousValue.page
            ...
            if (options.notify) {
                ...
                if (this.options.paginationChanged) {
                    yield this.options.paginationChanged(this, event);
                }
                this.handleOn('paginationChanged', event);
                if (reason === 'page') {
                    if (this.options.pageChanged) {
                        yield this.options.pageChanged(this, event);
                    }
                    this.handleOn('pageChanged', event);
                }
                else if (reason === 'limit') {
                    if (this.options.itemsPerPageChanged) {
                        yield this.options.itemsPerPageChanged(this, event);
                    }
    ...
    selectionValuesEqual(left, right) {
    ...
    assetValueFromRecords(records) {
        const ids = records
            .map((record) => nested_property_1.default.get(record, this.assetIdField()))
    ...
    buildAssetRecordMetadata(record) {
        const name = nested_property_1.default.get(record, this.assetNameField()) || this.$uiText('ve.field.assetLabel', 'Asset');
        const mimeType = nested_property_1.default.get(record, this.assetMimeTypeField()) || '';
        const size = nested_property_1.default.get(record, this.assetSizeField());
        const previewUrl = nested_property_1.default.get(record, this.assetPreviewField());
        const downloadUrl = nested_property_1.default.get(record, this.assetDownloadField());
        return {
            key: String(nested_property_1.default.get(record, this.assetIdField()) || name),
            label: String(name),
    ...
    hydrateAssetRecords(records) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return Promise.all(records.map((record) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (adapter.getPreviewUrl && !nested_property_1.default.get(nextRecord, this.assetPreviewField())) {
                    const previewUrl = yield adapter.getPreviewUrl(record, this);
                    if (previewUrl) {
                        nested_property_1.default.set(nextRecord, this.assetPreviewField(), previewUrl);
                    }
                }
                if (adapter.getDownloadUrl && !nested_property_1.default.get(nextRecord, this.assetDownloadField())) {
                    const downloadUrl = yield adapter.getDownloadUrl(record, this);
                    if (downloadUrl) {
                        nested_property_1.default.set(nextRecord, this.assetDownloadField(), downloadUrl);
                    }
                }
                return nextRecord;
            })));
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const file of files) {
                if (maxSize > 0 && file.size > (maxSize * 1024)) {
                    dialogs_1.Dialogs.$error(this.$uiText('ve.validation.fileMaxSize', '{file} exceeds the maximum allowed size of {max} KB.', { file: file.name, max: maxSize }));
                    continue;
                }
    ...
    createDirectUploadValue(files) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (mediaType === 'image' || mediaType === 'document') {
                const data = [];
                for (const file of files) {
                    data.push(yield (0, misc_1.fileToBase64)(file, this.params.value.fileMaxSize || 500));
                }
            ...
            const encoded = [];
            for (const file of files) {
                encoded.push(yield (0, misc_1.fileToBase64)(file, this.params.value.fileMaxSize || 500));
            }
    ...
    $uploadAssets() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                dialogs_1.Dialogs.$error((error === null || error === void 0 ? void 0 : error.message) || this.$uiText('ve.field.assetUploadFailed', 'Failed to upload asset files.'));
                throw error;
            }
            finally {
    ...
    handleSelectedFiles(files) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                dialogs_1.Dialogs.$error((error === null || error === void 0 ? void 0 : error.message) || this.$uiText('ve.field.fileProcessFailed', 'Failed to process selected files.'));
            }
        });
    }
    syncResolvedAssets() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                dialogs_1.Dialogs.$error((error === null || error === void 0 ? void 0 : error.message) || this.$uiText('ve.field.assetResolveFailed', 'Failed to resolve asset references.'));
            }
        });
    }
    clearMediaValue() {
    ...
    openMediaItem(item) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof source === 'string' || (typeof Blob !== 'undefined' && source instanceof Blob)) {
                yield dialogs_1.Dialogs.$previewFile(source, {
    ...
    notifyChanged(context) {
        if (this.options.changed)
            this.options.changed(this, context);
        this.handleOn('changed', context.value, context);
    }
    notifyInitialized(context, force = false) {
        if (this.initialized && !force) {
            return;
        }
        this.initialized = true;
        const version = ++this.initializationVersion;
        const dispatch = () => __awaiter(this, void 0, void 0, function* () {
            yield (0, vue_1.nextTick)();
            if (version !== this.initializationVersion) {
                return;
            }
            if (this.options.initialized) {
                yield this.options.initialized(this, context);
            }
            if (version !== this.initializationVersion) {
                return;
            }
    ...
    valueChanged(newValue, origin = 'programmatic', previousValue) {
    ...
    masterChangeAffectsValue(event) {
    ...
    attachEventListeners() {
        if (this.$master) {
            this.$master.on('changed', (event) => this.synchronizeValue({
    ...
    decimalPlaces() {
        return (0, decimal_1.resolveDecimalPlaces)(this.params.value.decimalPlaces);
    }
    validateDecimalValue(value) {
        ...
        for (const item of values) {
            const result = (0, decimal_1.normalizeExactDecimal)(item, this.decimalPlaces());
            if (result.empty) {
                continue;
            }
            if (!result.valid) {
            ...
            if (result.exceedsScale) {
                return this.$uiText('ve.validation.decimalPlaces', 'Value cannot exceed {places} decimal places.', { places: this.decimalPlaces() });
            }
        }
        return undefined;
    }
    decimalComparisonRule(expected, operator, key, fallback, values) {
        return (value) => {
            ...
            for (const candidate of candidates) {
                ...
                const comparison = (0, decimal_1.compareExactDecimals)(candidate, expected);
    ...
    synchronizeValue(options = {}, forceInitialization = false) {
        if (!this.changing) {
            const previousValue = this.modelValue.value;
    ...
    renderMathInHtml(html, output = 'htmlAndMathml') {
        ...
        html = html.replace(/\$\$([\s\S]+?)\$\$/g, (_, math) => {
            try {
                return katex_1.default.renderToString(math, { displayMode: true, output });
            }
            catch (err) {
        ...
        html = html.replace(/\$([^\$]+?)\$/g, (_, math) => {
            try {
                return katex_1.default.renderToString(math, { displayMode: false, output });
            }
            catch (err) {
                return `<span class="katex-error">${math}</span>`;
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (this.params.value.type === 'code' && this.params.value.lang === 'latex') {
                try {
                    ...
                    const webtex = yield Promise.resolve().then(() => __importStar(require('webtex')));
                    let fulltext = value || '';
    ...
    preprocess(value) {
        ...
        if (this.params.value.type === 'decimal') {
            const values = Array.isArray(value) ? value : [value];
            const normalized = values.map((item) => (0, decimal_1.normalizeExactDecimal)(item, this.decimalPlaces()).value);
    ...
    parseCompactDateValue(value) {
        ...
        if (!/^\d{8}$/.test(compact)) {
            return new misc_1.SimpleDate(value);
        }
        const year = Number(compact.slice(0, 4));
        const month = Number(compact.slice(4, 6));
        const day = Number(compact.slice(6, 8));
        return new misc_1.SimpleDate(`${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`);
    }
    ...
    preprocessTimeValue(value) {
        ...
        if (this.resolvedTimeFormat() === 'HHMM') {
            ...
            if (/^\d{3,4}$/.test(compact)) {
                ...
                return new misc_1.SimpleTime(`${padded.slice(0, 2)}:${padded.slice(2, 4)}`).toString();
            }
        }
    ...
    postprocessDateValue(value) {
        ...
        const date = new misc_1.SimpleDate(value);
    ...
    postprocessTimeValue(value) {
        ...
        const time = new misc_1.SimpleTime(value);
    ...
    makeHTMLColumns(headers) {
        ...
        const addColumns = (columns) => {
            ...
            for (const header of columns) {
                ...
                if ((_a = header.children) === null || _a === void 0 ? void 0 : _a.length) {
    ...
    syncAutocompleteSelectionDisplay() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.isAutocompleteTable()) {
                yield this.syncAutocompleteTableSelection();
                return;
            }
    ...
    autocompleteItemKey(item, index) {
        if (item && typeof item === 'object') {
            const itemId = master_1.Master.getItemId(item, this.params.value.itemValue || this.params.value.idField);
            if (itemId || itemId === 0) {
    ...
    autocompleteItemMatchesValue(item, value) {
        if (item && typeof item === 'object') {
            return master_1.Master.matchesItemId(item, value, this.params.value.itemValue || this.params.value.idField);
        }
    ...
    autocompleteValuesEqual(left, right) {
        ...
        const leftId = normalizedLeft && typeof normalizedLeft === 'object'
            ? master_1.Master.getItemId(normalizedLeft, itemField)
            : normalizedLeft;
        const rightId = normalizedRight && typeof normalizedRight === 'object'
            ? master_1.Master.getItemId(normalizedRight, itemField)
            : normalizedRight;
    ...
    autocompleteDisplayTitle(item) {
        ...
        const title = nested_property_1.default.get(resolvedItem, titleField);
    ...
    autocompleteTableStoredValues() {
        return this.normalizeAutocompleteItems(this.modelValue.value);
    }
    autocompleteTableFallbackItem(value) {
        const itemValue = this.params.value.itemValue || this.params.value.idField || master_1.Master.getDefaultIdField() || '_id';
        const itemTitle = this.params.value.itemTitle || 'name';
        const item = { __veAutocompleteUnresolved: true };
        nested_property_1.default.set(item, itemValue, value);
        nested_property_1.default.set(item, itemTitle, String(value !== null && value !== void 0 ? value : ''));
        return item;
    }
    autocompleteTableStoredValue(item) {
        const normalized = this.normalizeAutocompleteComparisonValue(item);
        if (this.params.value.returnObject) {
            return normalized;
        }
        if (normalized && typeof normalized === 'object') {
            return master_1.Master.getItemId(normalized, this.params.value.itemValue || this.params.value.idField);
        }
        return normalized;
    }
    resolveAutocompleteTableItems(requestId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let candidates = this.mergeAutocompleteItems(this.autocompleteResolvedItems.value || [], this.mergeAutocompleteItems(this.autocompleteResultItems.value || [], this.selectItems.value || []));
            const missingValues = storedValues.filter((value) => !candidates.some((item) => this.autocompleteItemMatchesValue(item, value)));
            if (missingValues.length > 0 && this.options.autocompleteResolveValue) {
                try {
                    const resolved = yield this.options.autocompleteResolveValue(this, missingValues, {});
                    if (requestId !== this.autocompleteTableRequestId.value) {
                        return [];
                    }
                    const resolvedItems = this.normalizeAutocompleteItems(resolved);
                    candidates = this.mergeAutocompleteItems(candidates, resolvedItems);
                    this.updateResolvedAutocompleteItems(this.mergeAutocompleteItems(this.autocompleteResolvedItems.value || [], resolvedItems));
                }
                catch (error) {
                    if ((error === null || error === void 0 ? void 0 : error.name) === 'AbortError') {
                        return [];
            ...
            const orderedItems = storedValues.map((value) => candidates.find((item) => this.autocompleteItemMatchesValue(item, value))
                || this.autocompleteTableFallbackItem(value));
            this.updateResolvedAutocompleteItems(this.mergeAutocompleteItems(this.autocompleteResolvedItems.value || [], orderedItems.filter((item) => !(item === null || item === void 0 ? void 0 : item.__veAutocompleteUnresolved))));
            return orderedItems;
        });
    }
    loadAutocompleteTableHeaders() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.autocompleteTableHeadersLoaded.value) {
                return;
            }
            const headers = yield this.headers();
            this.autocompleteTableHeaders.value = (headers === null || headers === void 0 ? void 0 : headers.length)
                ? headers
    ...
    autocompleteTableSourceKey(item, index) {
    ...
    buildAutocompleteTableRows(items) {
        ...
        return formattedItems.map((formattedItem, index) => {
            ...
            const selectionKey = `selection:${(_a = source === null || source === void 0 ? void 0 : source.index) !== null && _a !== void 0 ? _a : index}:${formattedKey}`;
            const displayItem = formattedItem && typeof formattedItem === 'object'
                ? Object.assign({}, formattedItem) : { [this.params.value.itemTitle || 'name']: String(formattedItem !== null && formattedItem !== void 0 ? formattedItem : '') };
    ...
    syncAutocompleteTableSelection() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield this.loadAutocompleteTableHeaders();
                const items = yield this.resolveAutocompleteTableItems(requestId);
                if (requestId !== this.autocompleteTableRequestId.value) {
                    return;
                }
    ...
    addAutocompleteTableItem() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const item = this.autocompleteTablePendingRawItem();
            const storedValue = this.autocompleteTableStoredValue(item);
            const appended = (0, field_autocomplete_table_state_1.appendUniqueAutocompleteValue)(this.autocompleteTableStoredValues(), storedValue, (left, right) => this.autocompleteValuesEqual(left, right));
            if (!appended.added) {
                return;
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const key of this.autocompleteTableSelectedKeys.value) {
                ...
                const canRemove = this.options.canRemoveItem
                    ? yield this.options.canRemoveItem(this, source.item)
                    : true;
                if (canRemove !== false) {
            ...
            const nextValue = (0, field_autocomplete_table_state_1.removeAutocompleteValuesAtIndexes)(this.autocompleteTableStoredValues(), removableIndexes);
    ...
    applyServerAutocompleteSearch(search, page = 1, options) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                yield this.syncAutocompleteSelectionDisplay();
    ...
    autocompleteTableText(value, key, fallback, values) {
        if ((0, runtime_1.isUITextDescriptor)(value)) {
            return (0, runtime_1.resolveUIText)(Object.assign(Object.assign({}, value), { values: Object.assign(Object.assign({}, (value.values || {})), (values || {})) }), fallback);
        }
        if (value !== undefined && value !== null) {
            return (0, runtime_1.interpolateUITextTemplate)(this.$text(value), values);
        }
        return this.$uiText(key, fallback, values);
    }
    autocompleteTableAddText() {
        return this.autocompleteTableText(this.params.value.autocompleteAddText, 've.field.autocomplete.add', 'Add');
    }
    autocompleteTableSelectedText() {
        return this.autocompleteTableText(this.params.value.autocompleteSelectedText, 've.field.autocomplete.selected', 'Selected {label} ({count})', {
            count: this.autocompleteTableStoredValues().length,
            label: this.resolvedLabel() || this.$uiText('ve.field.autocomplete.items', 'items'),
        });
    }
    autocompleteTableRemoveText() {
        return this.autocompleteTableText(this.params.value.autocompleteRemoveText, 've.field.autocomplete.removeSelected', 'Remove selected items', { count: this.autocompleteTableSelectedKeys.value.length });
    }
    resolvedLabel() {
    ...
    validate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((0, runtime_1.isUIValidationMessage)(decimalResult))
                return decimalResult;
            const validationVersion = ++this.htmlValidationVersion;
            const value = this.modelValue.value;
            const result = this.options.validate ? yield this.options.validate(this) : undefined;
    ...
    rules() {
        const items = this.options.rules ? this.options.rules(this) : [];
        if (this.params.value.type === 'decimal') {
            items.unshift((value) => this.validateDecimalValue(value) || true);
        }
        ...
        if (this.params.value.validation) {
            ...
            if (v.range) {
                items.push(this.params.value.type === 'decimal'
                    ...
                    : misc_2.$v.max(v.range.to, v.range.converter || converter));
                items.push(this.params.value.type === 'decimal'
                    ? this.decimalComparisonRule(v.range.from, 'gte', 've.validation.min', 'Value cannot be below {min}', { min: v.range.from })
                    : misc_2.$v.min(v.range.from, v.range.converter || converter));
            }
            if (v.max)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.max.value, 'lte', 've.validation.max', 'Value cannot exceed {max}', { max: v.max.value }) : misc_2.$v.max(v.max.value, v.max.converter || converter));
            if (v.min)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.min.value, 'gte', 've.validation.min', 'Value cannot be below {min}', { min: v.min.value }) : misc_2.$v.min(v.min.value, v.min.converter || converter));
            if (v.gt)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.gt.value, 'gt', 've.validation.greaterThan', 'Value should be greater than {value}', { value: v.gt.value }) : misc_2.$v.gt(v.gt.value, v.gt.converter || converter));
            if (v.lt)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.lt.value, 'lt', 've.validation.lessThan', 'Value should be less than {value}', { value: v.lt.value }) : misc_2.$v.lt(v.lt.value, v.lt.converter || converter));
            if (v.gte)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.gte.value, 'gte', 've.validation.greaterThanOrEqual', 'Value should be greater than or equal to {value}', { value: v.gte.value }) : misc_2.$v.gte(v.gte.value, v.gte.converter || converter));
            if (v.lte)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.lte.value, 'lte', 've.validation.lessThanOrEqual', 'Value should be less than or equal to {value}', { value: v.lte.value }) : misc_2.$v.lte(v.lte.value, v.lte.converter || converter));
            if (v.neq)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.neq.value, 'neq', 've.validation.notEqual', 'Value cannot be equal to {value}', { value: v.neq.value }) : misc_2.$v.neq(v.neq.value, v.neq.converter || converter));
            if (v.eq)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.eq.value, 'eq', 've.validation.equal', 'Value must be equal to {value}', { value: v.eq.value }) : misc_2.$v.eq(v.eq.value, v.eq.converter || converter));
            if (v.in)
    ...
    resolveValidationRule(rule) {
        ...
        return (value) => {
            ...
            if (result && typeof result.then === 'function') {
                return result.then((resolved) => (0, runtime_1.isUIValidationMessage)(resolved) ? (0, runtime_1.resolveUIText)(resolved) : resolved);
            }
            return (0, runtime_1.isUIValidationMessage)(result) ? (0, runtime_1.resolveUIText)(result) : result;
        };
    }
    build(props, context) {
    ...
    buildHTMLView(props, context) {
        ...
        return h('div', {
            ...
            onClick: (event) => this.dispatchHtmlViewEvent(event, 'click'),
            onChange: (event) => this.dispatchHtmlViewEvent(event, 'change'),
            onInput: (event) => this.dispatchHtmlViewEvent(event, 'input'),
            onSubmit: (event) => this.dispatchHtmlViewEvent(event, 'submit'),
        });
    }
    parseHtmlEventPayload(value) {
        if (value === null || value === '') {
    ...
    htmlEventElementValue(element) {
        if ((element === null || element === void 0 ? void 0 : element.type) === 'checkbox' || (element === null || element === void 0 ? void 0 : element.type) === 'radio') {
            return !!element.checked;
        }
        return element && 'value' in element ? element.value : undefined;
    }
    dispatchHtmlViewEvent(nativeEvent, eventType) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            const target = nativeEvent.target;
            const currentTarget = nativeEvent.currentTarget;
            const element = (_a = target === null || target === void 0 ? void 0 : target.closest) === null || _a === void 0 ? void 0 : _a.call(target, '[data-ve-event]');
            if (!element || ((currentTarget === null || currentTarget === void 0 ? void 0 : currentTarget.contains) && !currentTarget.contains(element))) {
                return;
            }
            ...
            if (this.options.htmlEvent) {
                yield this.options.htmlEvent(this, event);
            }
    ...
    buildPagination(_props, _context) {
        return (0, field_pagination_widget_1.buildPaginationWidget)(this);
    }
    get $popupReady() {
    ...
    popupLifecycleProps() {
        ...
        const ownerChainMatches = !!request && this.popupOwners.length === owners.length &&
            request.generations.length === owners.length && owners.every((entry, index) => this.popupOwners[index] === entry.owner && request.generations[index] === entry.generation);
        return {
            ...
            menu: owners.every(owner => owner.ready) && ownerChainMatches && (request === null || request === void 0 ? void 0 : request.open) === true,
            'onUpdate:menu': (open) => {
    ...
    buildSelect(props, context) {
        var _a;
        const h = this.$h;
        return h(components_1.VSelect, Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, this.popupLifecycleProps()), this.modelBinding()), this.inputIconProps()), (this.params.value.menuTransition !== undefined ? { transition: this.params.value.menuTransition } : {})), { autofocus: this.params.value.autofocus, label: this.resolvedLabel(), hint: this.resolvedHint(), persistentHint: !!this.params.value.hint, placeholder: this.resolvedPlaceholder(), clearable: this.params.value.clearable || false, color: this.params.value.color || "primary", variant: this.params.value.variant || ((_a = Field.defaultParams) === null || _a === void 0 ? void 0 : _a.variant), itemTitle: this.params.value.itemTitle || 'name', itemValue: master_1.Master.resolveItemValueField(this.selectItems.value, this.params.value.itemValue || this.params.value.idField), readonly: this.$readonly, items: this.selectItems.value, multiple: this.params.value.multiple, class: this.params.value.class || [], style: this.params.value.style || {}, rules: this.rules(), "onUpdate:focused": (ev) => this.onFocusChanged(ev) }));
    }
    buildRadioSelect(props, context) {
        ...
        return [
            ...
            h(components_1.VRadioGroup, Object.assign(Object.assign({}, this.modelBinding()), { autofocus: this.params.value.autofocus, label: this.resolvedLabel(), hint: this.resolvedHint(), persistentHint: !!this.params.value.hint, placeholder: this.resolvedPlaceholder(), clearable: this.params.value.clearable || false, color: this.params.value.color || "primary", variant: this.params.value.variant || ((_a = Field.defaultParams) === null || _a === void 0 ? void 0 : _a.variant), readonly: this.$readonly, class: ['vef-radio-select'].concat(this.params.value.class || []), style: this.params.value.style || {}, rules: this.rules(), inline: this.params.value.inline, "onUpdate:focused": (ev) => this.onFocusChanged(ev) }), () => (this.selectItems.value || []).map((item) => h(components_1.VRadio, {
                value: master_1.Master.getItemId(item, this.params.value.itemValue || this.params.value.idField),
    ...
    buildCheckboxSelect(props, context) {
        ...
        return [
            ...
            ...(this.selectItems.value || []).map((item) => h(components_1.VCheckboxBtn, Object.assign(Object.assign({ value: master_1.Master.getItemId(item, this.params.value.itemValue || this.params.value.idField) }, this.modelBinding()), { readonly: this.$readonly, class: ['vef-check-select'].concat(this.params.value.class || []), style: this.params.value.style || {}, color: this.params.value.color || "primary", hint: this.resolvedHint(), multiple: this.params.value.multiple, persistentHint: !!this.params.value.hint, inline: this.params.value.inline }), {
                label: () => h('div', Object.assign(Object.assign({}, (item.props || {})), { innerHTML: item[this.params.value.itemTitle || 'name'] }))
            }))
        ];
    }
    autocompleteServerInputProps() {
        if (!this.isServerAutocomplete()) {
            return {};
        }
    ...
    buildAutocomplete(props, context) {
        ...
        return h(components_1.VAutocomplete, Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, this.modelBinding()), this.inputIconProps()), { autofocus: this.params.value.autofocus, label: this.resolvedLabel(), hint: this.resolvedHint(), persistentHint: !!this.params.value.hint, placeholder: this.resolvedPlaceholder(), clearable: this.params.value.clearable || false, color: this.params.value.color || "primary", variant: this.params.value.variant || ((_a = Field.defaultParams) === null || _a === void 0 ? void 0 : _a.variant), itemTitle: this.params.value.itemTitle || 'name', itemValue: master_1.Master.resolveItemValueField(this.selectItems.value, this.params.value.itemValue || this.params.value.idField), readonly: this.$readonly, items: this.selectItems.value, loading: this.autocompleteLoading.value, autoSelectFirst: true, returnObject: this.params.value.returnObject, valueComparator: (left, right) => this.autocompleteValuesEqual(left, right), multiple: this.params.value.multiple }), serverAutocompleteProps), this.popupLifecycleProps()), (this.params.value.menuTransition !== undefined ? {
            menuProps: Object.assign(Object.assign({}, serverAutocompleteProps.menuProps), { transition: this.params.value.menuTransition }),
        } : {})), { class: this.params.value.class || [], style: this.params.value.style || {}, rules: this.rules(), "onUpdate:focused": (ev) => this.onFocusChanged(ev) }), autocompleteSlots);
    }
    buildAutocompleteTable(props, context) {
        var _a;
        const h = this.$h;
        const serverAutocompleteProps = this.autocompleteServerInputProps();
        ...
        autocompleteSlots.append = () => h(components_1.VBtn, {
            color: this.params.value.color || 'primary',
            variant: 'tonal',
            size: 'small',
            icon: true,
        ...
        }, () => h(components_1.VIcon, { size: 22 }, () => 'mdi-plus'));
        const headers = (0, table_header_1.resolveUITableHeaders)(this.autocompleteTableHeaders.value, (value) => this.$text(value));
        ...
        return h('div', {
            ...
            ...(!this.$readonly ? [
                h(components_1.VAutocomplete, Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ modelValue: this.autocompleteTablePendingItem.value, "onUpdate:modelValue": (value) => {
                        this.autocompleteTablePendingItem.value = value;
                    } }, this.inputIconProps()), { autofocus: this.params.value.autofocus, label: this.resolvedLabel(), hint: this.resolvedHint(), persistentHint: !!this.params.value.hint, placeholder: this.resolvedPlaceholder(), clearable: this.params.value.clearable !== false, color: this.params.value.color || 'primary', variant: this.params.value.variant || ((_a = Field.defaultParams) === null || _a === void 0 ? void 0 : _a.variant), itemTitle: this.params.value.itemTitle || 'name', itemValue: master_1.Master.resolveItemValueField(this.selectItems.value, this.params.value.itemValue || this.params.value.idField), items: this.selectItems.value, loading: this.autocompleteLoading.value || this.optionLoading.value, autoSelectFirst: true, returnObject: true, valueComparator: (left, right) => this.autocompleteValuesEqual(left, right), rules: tableRules }), serverAutocompleteProps), this.popupLifecycleProps()), (this.params.value.menuTransition !== undefined ? {
                    menuProps: Object.assign(Object.assign({}, serverAutocompleteProps.menuProps), { transition: this.params.value.menuTransition }),
                } : {})), { class: 'vef-autocomplete-table__search', "onUpdate:focused": (ev) => this.onFocusChanged(ev) }), autocompleteSlots),
            ] : []),
            h('div', { class: 'vef-autocomplete-table__heading' }, [
                ...
                ...(allowRemove && selectedCount > 0 ? [
                    ...
                    }, () => h(components_1.VIcon, { size: 22 }, () => 'mdi-delete')),
                ] : []),
            ]),
    ...
    richWidgetContext() {
        return {
            ...
            htmlValidationMessage: () => (0, runtime_1.isUIValidationMessage)(this.htmlValidationResult.value)
            ...
            renderLatex: (value) => void this.renderLatex(value),
            ...
            selectMediaFiles: () => __awaiter(this, void 0, void 0, function* () {
                try {
                    const files = yield (0, misc_1.selectFile)(this.params.value.fileAccepts, !!this.params.value.multiple);
                    yield this.handleSelectedFiles(files);
                }
                catch (error) {
                    if ((error === null || error === void 0 ? void 0 : error.message) !== 'No File Selected!') {
                        dialogs_1.Dialogs.$error((error === null || error === void 0 ? void 0 : error.message) || this.$uiText('ve.field.fileSelectFailed', 'Failed to select files.'));
                    }
                }
            }),
    ...
    focusHtmlEditor() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const body = (_a = editor.getBody) === null || _a === void 0 ? void 0 : _a.call(editor);
            ...
            if (typeof ((_c = editor.selection) === null || _c === void 0 ? void 0 : _c.select) === 'function' && typeof ((_d = editor.selection) === null || _d === void 0 ? void 0 : _d.collapse) === 'function') {
    ...
    buildCollection(props, context) {
        const h = this.$h;
        const headers = (0, table_header_1.resolveUITableHeaders)(this.collectionHeaders, (value) => this.$text(value));
        ...
        return h(components_1.VRow, {}, () => [
            ...
            ...(this.collectionLoaded.value ? [
                h(components_1.VCol, {
                    ...
                    ref: (value) => { this.collectionOverlay = value; },
                    modelValue: this.collectionDialog.value,
                    onBeforeEnter: () => { this.collectionPopupReady.value = false; },
                    onAfterEnter: () => __awaiter(this, void 0, void 0, function* () {
                        var _b;
                        const generation = this.collectionPopupGeneration;
                        if ((yield (0, dialog_lifecycle_1.finishDialogEntry)((_b = this.collectionOverlay) === null || _b === void 0 ? void 0 : _b.contentEl)) &&
    ...
    buildFileUpload(props, context) {
        ...
        return h(components_1.VRow, {}, () => [
            ...
            ...(items.length > 0 ? [
                h(components_1.VCol, {
                    cols: 12,
                }, () => items.map((item, index) => h(components_1.VCard, {
                    key: item.key,
                    class: ['mb-2'],
                    variant: 'outlined',
                }, () => h('div', {
    ...
    showFullscreen(data) {
        void dialogs_1.Dialogs.$previewFile(data, {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.$master && this.params.value.storage) {
                ...
                if (fm.$params.mode === 'edit') {
                    const valueId = master_1.Master.getItemId(value, this.params.value.idField);
                    this.$master.$setCollectionObject(this.params.value.storage, (valueId || valueId === 0) ? valueId : (_b = value.__index) === null || _b === void 0 ? void 0 : _b.toString(), value, (valueId || valueId === 0) ? master_1.Master.resolveItemValueField([value], this.params.value.idField) : '__index');
                }
                this.updateValue();
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (this.$master && this.params.value.storage) {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    if (canRemove) {
                        const itemId = master_1.Master.getItemId(items[i], this.params.value.idField);
                        this.$master.$removeCollectionObject(this.params.value.storage, (itemId || itemId === 0) ? itemId : (items[i].__index || items[i].__index === 0 ? items[i].__index.toString() : items[i].toString()), (itemId || itemId === 0) ? master_1.Master.resolveItemValueField([items[i]], this.params.value.idField) : '__index');
                    }
                }
    ...
    onCollectionItemClicked(item) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const value = this.currentCollectionItems.filter((i) => master_1.Master.matchesItemId(i, itemId, this.params.value.idField) || i.__index === item.__index)[0];
    ...
    buildTable(props, context) {
        return (0, field_table_widgets_1.buildTableWidget)(this.tableWidgetContext());
    }
    forceLoadTableInfo() {
    ...
    buildServerTable(props, context) {
        return (0, field_table_widgets_1.buildServerTableWidget)(this.tableWidgetContext());
    }
    buildViewTable(props, context) {
        return (0, field_table_widgets_1.buildViewTableWidget)(this.tableWidgetContext());
    }
    buildReportTable(props, context) {
        return (0, field_table_widgets_1.buildReportTableWidget)(this.tableWidgetContext());
    }
        ...
        if (this.isEditting && !focused) {
            if (this.params.value.type === 'decimal' && this.modelValue.value !== undefined && this.modelValue.value !== null) {
                ...
                try {
                    ...
                    const dvalue = values.map((value) => (0, decimal_1.normalizeExactDecimal)(value, this.decimalPlaces()).value);
