...
class Form extends base_1.UIBase {
    ...
    fieldFocusBehavior(field) {
        ...
        if (['label', 'htmlview', 'button', 'chart', 'map', 'map-polygon', 'table', 'viewtable', 'servertable', 'reporttable', 'collection', 'messagingbox', 'image', 'document'].includes(type)) {
            return 'skip';
        }
        if (type === 'html') {
            return 'special';
    ...
    buildDefaultButtons() {
        const prevButtons = this.params.value.prevButton ? [
            new button_1.Button(Object.assign({ text: 'Prev', color: 'secondary' }, (this.params.value.prevButton || {})), {
