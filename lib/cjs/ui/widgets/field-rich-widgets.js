...
const ace = __importStar(require("ace-builds"));
    ...
    if (isPolygonMap(field)) {
        ...
        if (polygonPaths.length >= 3) {
            ...
            polygonPaths.forEach((item) => {
                mapChildren.push(h(vue3_google_map_1.Marker, { key: `${pointKey(item)}-draft-polygon`, options: { position: item, draggable: false } }));
            });
        }
    }
        ...
        linePaths.forEach((item) => {
            mapChildren.push(h(vue3_google_map_1.Marker, { key: `${pointKey(item)}-line`, options: { position: item, draggable: false } }));
        });
    }
