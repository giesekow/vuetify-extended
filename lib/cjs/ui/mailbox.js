...
class Mailbox {
    ...
    static get $title() {
        return (0, runtime_1.resolveUIText)(Mailbox.options.value.title, 'Mailbox');
    }
    ...
    static loadPage(input) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                dialogs_1.Dialogs.$error((error === null || error === void 0 ? void 0 : error.message) || { key: 've.mailbox.loadFailed', fallback: 'Failed to load mailbox items.' });
                return Mailbox.items.value;
            }
            finally {
...
Mailbox.options = (0, vue_1.ref)({ pageSize: 8, title: { key: 've.mailbox.title', fallback: 'Mailbox' } });
...
class MailboxView extends base_1.UIBase {
    ...
    clampToViewport(value, fallback) {
        const size = this.normalizeSize(value !== null && value !== void 0 ? value : fallback);
        if (!size) {
            return undefined;
        }
    ...
    deleteSelected() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const accepted = yield dialogs_1.Dialogs.$confirm({
        ...
        return h(components_1.VContainer, { fluid: this.$params.fluid }, () => h(components_1.VLayout, { fullHeight: true }, () => h(components_1.VRow, { justify, align }, () => h(components_1.VCol, { cols: 12, style: { paddingTop: '16px', paddingBottom: '16px' } }, () => h('div', { style: { width: '100%', display: 'flex', justifyContent: this.$params.horizontalAlign === 'left' ? 'flex-start' : this.$params.horizontalAlign === 'right' ? 'flex-end' : 'center' } }, [
            h(components_1.VCard, { elevation: 4, style: { width, maxWidth: '100%', overflow: 'hidden' } }, () => [
                h(components_1.VCardTitle, { style: { display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' } }, () => [
                    ...
                    h('span', title),
                    h(components_1.VChip, { size: 'small', color: Mailbox.$unreadCount > 0 ? 'primary' : 'default', variant: Mailbox.$unreadCount > 0 ? 'elevated' : 'outlined' }, () => this.$uiText('ve.mailbox.unreadCount', '{count} unread', { count: Mailbox.$unreadCount })),
                    ...(selectedCount > 0 ? [h(components_1.VChip, { size: 'small', color: 'secondary', variant: 'outlined' }, () => this.$uiText('ve.mailbox.selectedCount', '{count} selected', { count: selectedCount }))] : []),
                    h(components_1.VSpacer),
                    h(components_1.VBtn, { icon: 'mdi-refresh', variant: 'text', title: this.$uiText('ve.common.refresh', 'Refresh'), 'aria-label': this.$uiText('ve.common.refresh', 'Refresh'), onClick: () => __awaiter(this, void 0, void 0, function* () { this.clearSelection(); yield Mailbox.refresh(this.$params.pageSize); }) }),
                    h(components_1.VBtn, { icon: 'mdi-close', variant: 'text', title: this.$uiText('ve.common.close', 'Close'), 'aria-label': this.$uiText('ve.common.close', 'Close'), onClick: () => this.forceCancel() }),
                ]),
                ...(selectedCount > 0 ? [
                    h(components_1.VDivider),
                    h(components_1.VCardActions, { style: { padding: '8px 16px', gap: '8px', flexWrap: 'wrap' } }, () => [
                        h(components_1.VBtn, { variant: 'outlined', color: 'primary', prependIcon: 'mdi-email-open-outline', onClick: () => this.markSelectedRead() }, () => this.$uiText('ve.mailbox.markSelectedRead', 'Mark Selected Read')),
                        h(components_1.VBtn, { variant: 'outlined', color: 'error', prependIcon: 'mdi-delete-outline', onClick: () => this.deleteSelected() }, () => this.$uiText('ve.mailbox.deleteSelected', 'Delete Selected')),
                        h(components_1.VBtn, { variant: 'text', onClick: () => this.clearSelection() }, () => this.$uiText('ve.mailbox.clearSelection', 'Clear Selection')),
                    ]),
                ] : []),
                h(components_1.VDivider),
                h(components_1.VCardText, { style: { padding: '0px' } }, () => [
                    Mailbox.$items.length === 0 && !Mailbox.$loading
                        ...
                        : h('div', { style: { maxHeight: listMaxHeight, overflowY: 'auto' } }, [
                            h(components_1.VList, { lines: 'three' }, () => Mailbox.$items.map((item) => h(components_1.VListItem, {
                                ...
                                title: () => h(components_1.VListItemTitle, { style: { fontWeight: item.read ? '500' : '700' } }, () => (0, runtime_1.resolveUIText)(item.title)),
                                subtitle: () => h('div', {}, [
                                    h(components_1.VListItemSubtitle, {}, () => (0, runtime_1.resolveUIText)(item.text)),
                                    h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', marginTop: '6px', flexWrap: 'wrap' } }, [
                                        ...(item.category ? [h(components_1.VChip, { size: 'x-small', variant: 'outlined', color: item.read ? undefined : 'primary' }, () => (0, runtime_1.resolveUIText)(item.category))] : []),
                                ...
                                append: () => h('div', { style: { display: 'flex', alignItems: 'center', gap: '4px' } }, [
                                    h(components_1.VBtn, { icon: item.read ? 'mdi-email-marked-unread' : 'mdi-email-open-outline', size: 'small', variant: 'text', title: item.read ? this.$uiText('ve.mailbox.markUnread', 'Mark unread') : this.$uiText('ve.mailbox.markRead', 'Mark read'), 'aria-label': item.read ? this.$uiText('ve.mailbox.markUnread', 'Mark unread') : this.$uiText('ve.mailbox.markRead', 'Mark read'), onClick: (ev) => { ev.stopPropagation(); item.read ? Mailbox.markUnread(item) : Mailbox.markRead(item); } }),
                                    h(components_1.VBtn, { icon: 'mdi-delete-outline', size: 'small', variant: 'text', title: this.$uiText('ve.common.remove', 'Remove'), 'aria-label': this.$uiText('ve.common.remove', 'Remove'), onClick: (ev) => { ev.stopPropagation(); Mailbox.remove(item); this.selectedIds.value = this.selectedIds.value.filter((id) => id !== item.id); } }),
                                ]),
                            })))
                        ]),
                    h(components_1.VDivider),
                    h(components_1.VCardActions, { style: { justifyContent: 'space-between', padding: '12px 16px', gap: '8px', flexWrap: 'wrap' } }, () => [
                        ...
                        h('div', { style: { display: 'flex', gap: '8px', flexWrap: 'wrap' } }, [
                            h(components_1.VBtn, { variant: 'text', disabled: Mailbox.$unreadCount === 0, onClick: () => Mailbox.setUnread(0) }, () => this.$uiText('ve.mailbox.clearUnreadBadge', 'Clear Unread Badge')),
                            h(components_1.VBtn, { variant: 'outlined', disabled: !Mailbox.$hasMore || Mailbox.$loading, onClick: () => Mailbox.loadMore(this.$params.pageSize) }, () => Mailbox.$loading ? this.$uiText('ve.common.loading', 'Loading...') : this.$uiText('ve.mailbox.loadMore', 'Load More')),
                        ]),
                    ]),
                ]),
            ]),
