...
class Notifications {
    ...
    static renderItem(item) {
        ...
        return hNode(components_1.VCard, {
            key: item.id,
            color: item.color,
            variant: 'elevated',
            elevation: 10,
            ...
            hNode(components_1.VCardTitle, {
                ...
                ...(item.icon ? [hNode(components_1.VIcon, { icon: item.icon, size: 20 })] : []),
                hNode('span', {
                    style: {
                        fontSize: '0.95rem',
                        fontWeight: 600,
                    },
                }, (0, runtime_1.resolveUIText)(item.title, 'Notification')),
                hNode(components_1.VSpacer),
                ...(item.closable === false ? [] : [
                    hNode(components_1.VBtn, {
                        icon: 'mdi-close',
                        size: 'small',
                        variant: 'text',
            ...
            hNode(components_1.VCardText, {
                style: {
                    paddingTop: item.title ? '0px' : '16px',
                    fontSize: '0.9rem',
                    lineHeight: 1.4,
                },
            }, () => (0, runtime_1.resolveUIText)(item.text)),
            ...(item.actions && item.actions.length > 0 ? [
                hNode(components_1.VCardActions, {
                    style: {
                        display: 'flex',
                        gap: '8px',
    ...
    static normalizeActions(actions) {
        ...
        return actions.map((button) => {
            if (appmanager_1.AppManager.$app) {
                button.setParent(appmanager_1.AppManager.$app);
            }
            return (0, vue_1.markRaw)(button);
        });
    }
...
Notifications.items = (0, vue_1.ref)([]);
Notifications.options = (0, vue_1.ref)({
    location: 'top-right',
    maxVisible: 4,
    defaultTimeout: 4000,
    surfaceStyle: 'opaque',
