...
class Trigger extends base_1.UIBase {
    ...
    buildSubTitle(props, context) {
        const h = this.$h;
        return h(components_1.VCardSubtitle, {}, () => h('span', {}, this.$text(this.params.value.subtitle)));
    }
    ...
    buildResultTable() {
        ...
        return [
            h(components_1.VCol, {
                ...
                headers: (0, table_header_1.resolveUITableHeaders)(this.computedHeaders.value, (value) => this.$text(value)),
    ...
    buildDefaultButtons() {
        if (this.hasRemoveAccess.value && this.selected.value && this.selected.value.length > 0) {
            return this.getAdditionalButtons().concat([
                new button_1.Button(Object.assign({ text: this.$uiText('ve.common.remove', 'Remove'), color: 'warning' }, (this.params.value.removeButton || {})), {
                    onClicked: () => this.onRemoveClicked()
                }),
                ...(this.params.value.multiple && this.selected.value && this.selected.value.length > 0 ? [
                    new button_1.Button(Object.assign({ text: this.params.value.mode === 'display' ? this.$uiText('ve.common.view', 'View') : this.$uiText('ve.common.edit', 'Edit'), color: 'success' }, (this.params.value.mode === 'display' ? this.params.value.viewButton || {} : this.params.value.editButton || {})), {
                        onClicked: () => this.onProcessMultiple()
                    })
                ] : []),
                new button_1.Button(Object.assign({ text: this.$uiText('ve.common.cancel', 'Cancel'), color: 'secondary' }, (this.params.value.cancelButton || {})), {
    ...
    onRemoveClicked() {
        return __awaiter(this, void 0, void 0, function* () {
            const confirm = yield dialogs_1.Dialogs.$confirm({ key: 've.trigger.confirmRemoveSelected', fallback: 'Remove selected items?' });
            if (!confirm)
                return;
            const items = this.selected.value || [];
            let hasError = false;
            for (let i = 0; i < items.length; i++) {
                ...
                if (canRem) {
                    ...
                    if (res !== true) {
                        dialogs_1.Dialogs.$warning(res || this.$uiText('ve.trigger.unableToRemoveItems', 'Unable to remove items'));
                        hasError = true;
                        break;
                    }
                }
            ...
            if (!hasError) {
                dialogs_1.Dialogs.$success(this.$uiText('ve.trigger.itemsRemoved', 'Items successfully removed!'));
            }
            this.selected.value = [];
