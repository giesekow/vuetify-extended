...
function configureVuetifyExtendedDefaults(defaults, reset) {
    ...
    if (defaults.confirm)
        dialogs_1.Dialogs.setConfirmDefault(defaults.confirm, reset);
    if (defaults.dialogForm)
        dialogform_1.DialogForm.setDefault(defaults.dialogForm, reset);
    if (defaults.documentPreview)
        dialogs_1.Dialogs.setDocumentPreviewDefault(defaults.documentPreview, reset);
    ...
    if (defaults.filePreview)
        dialogs_1.Dialogs.setFilePreviewDefault(defaults.filePreview, reset);
    if (defaults.form)
        form_1.Form.setDefault(defaults.form, reset);
    if (defaults.iframe)
        dialogs_1.Dialogs.setIframeDefault(defaults.iframe, reset);
    if (defaults.imagePreview)
        dialogs_1.Dialogs.setImagePreviewDefault(defaults.imagePreview, reset);
    if (defaults.info)
        dialogs_1.Dialogs.setInfoDefault(defaults.info, reset);
    ...
    if (defaults.prompt)
        dialogs_1.Dialogs.setPromptDefault(defaults.prompt, reset);
    if (defaults.report)
...
function createVuetifyExtendedApp(options = {}) {
    ...
    if (options.i18n) {
        (0, runtime_1.setVuetifyExtendedI18n)(options.i18n);
    }
    if (options.dialogs) {
