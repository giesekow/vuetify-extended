...
const nested_property_2 = __importDefault(require("nested-property"));
const runtime_1 = require("../ui/runtime");
class Master extends lib_1.EventEmitter {
    constructor(options) {
        super();
        this.data = {};
        this.validates = [];
    ...
    static getIdFieldCandidates(idField) {
        const candidates = [idField, Master.defaultOptions.idField, '_id', 'id'];
        return candidates.filter((field, index) => typeof field === 'string' && field !== '' && candidates.indexOf(field) === index);
    }
    static getValueByField(item, field) {
        if (!item || !field)
            return undefined;
        return nested_property_1.default.get(item, field);
    }
    static resolveItemValueField(items, idField) {
        const candidates = Master.getIdFieldCandidates(idField);
        const normalized = Array.isArray(items) ? items : (items ? [items] : []);
        for (const field of candidates) {
            if (normalized.some((item) => Master.getValueByField(item, field) !== undefined)) {
                return field;
            }
        }
        return candidates[0] || '_id';
    }
    static getItemId(item, idField) {
    ...
    static matchesItemId(item, id, idField) {
        if (!(id || id === 0))
            return false;
    ...
    validate(data) {
        return __awaiter(this, void 0, void 0, function* () {
            for (let v = 0; v < this.validates.length; v++) {
                ...
                if ((0, runtime_1.isUIValidationMessage)(r)) {
                    return r;
                }
            }
            return true;
    ...
    $getCollectionObject(key, id, idField) {
        ...
        if (!Array.isArray(items))
            items = [items];
        return items.filter((i) => Master.matchesItemId(i, id, idField || this.idField))[0];
    }
    $setCollectionObject(key, id, data, idField) {
        ...
        if (!id)
            return false;
        const item = items.filter((i) => Master.matchesItemId(i, id, idField || this.idField))[0];
        if (item) {
    ...
    saveSub() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if ((0, runtime_1.isUIValidationMessage)(val)) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.itemType && (this.itemId || mode === "create")) {
                ...
                for (const f of this.tempFields) {
                    if (nested_property_2.default.has(postprocessedData, f)) {
                        skiped.push({
                            key: f,
                            value: nested_property_2.default.get(postprocessedData, f)
                        });
                        nested_property_2.default.set(postprocessedData, f, null);
                    }
                }
                ...
                for (const sk of skiped) {
                    nested_property_2.default.set(postprocessedData, sk.key, sk.value);
                }
                this.data = Object.assign(Object.assign({}, postprocessedData), data);
                if (!this.itemId) {
    ...
    validationErrorMessage(value) {
        return (0, runtime_1.resolveUIText)({
            key: 've.validation.error',
            fallback: 'Validation Error: {message}',
            values: { message: (0, runtime_1.resolveUIValidationMessage)(value) || '' },
        });
    }
    $remove() {
        return __awaiter(this, void 0, void 0, function* () {
