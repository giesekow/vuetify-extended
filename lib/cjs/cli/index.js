...
function main(args = process.argv.slice(2)) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'report') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, create_ui_1.runCreateReportCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'route') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateRouteCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dialog-form') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateDialogFormCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'page') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreatePageCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'form') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, create_ui_1.runCreateFormCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'field') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreateFieldCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger-field') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateTriggerFieldCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger-action') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateTriggerActionCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'report-action') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateReportActionCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'part') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreatePartCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, create_ui_1.runCreateTriggerCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreateDashboardCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard-widget') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreateDashboardWidgetCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard-data-source') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateDashboardDataSourceCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'collection') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, create_ui_1.runCreateCollectionCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'service') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreateServiceCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'asset-service') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateAssetServiceCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'autocomplete-source') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateAutocompleteSourceCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'validator') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, advanced_ui_1.runCreateValidatorCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'menu-item') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, create_ui_1.runCreateMenuItemCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'header-item') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runCreateHeaderItemCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && (subcommand === 'sub-menu' || subcommand === 'submenu')) {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, create_ui_1.runCreateSubMenuCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'doctor') {
            ...
            return yield (0, advanced_ui_1.runDoctorCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'bootstrap' && subcommand === 'theme') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield (0, productivity_ui_1.runBootstrapThemeCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'migrate') {
            const parsed = parseNamedArgs(args.slice(1));
            return yield (0, advanced_ui_1.runMigrateCommand)({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
