...
import { computed, defineComponent, h as vueH, mergeProps, onBeforeUnmount, onMounted, ref, shallowRef, useId, watch } from 'vue';
...
const topLevelText = (key, fallback, values) => resolveUIText({ key, fallback, values }, fallback);
...
function withEditorAccessibilityAttributes(editorProps, attributes) {
    return Object.assign(Object.assign({}, (editorProps || {})), { attributes });
}
...
export const TiptapHtmlEditor = defineComponent({
    ...
    setup(props, { emit, attrs }) {
        ...
        const editorActionFrame = ref(null);
        const initHandlers = [];
        const keydownHandlers = [];
        const pendingEditorActions = [];
        const lastSelection = ref(undefined);
        const toolbarCompact = ref(false);
        const enabledToolbarItems = computed(() => new Set(resolveHtmlEditorToolbar(props.profile, props.toolbar, props.allowFullscreen)));
        ...
        const t = (key, fallback, values) => resolveUIText({ key, fallback, values }, fallback);
        ...
        const createIconButton = (params) => vueH(VTooltip, {
            text: params.title,
            'aria-label': params.title,
            location: 'top',
            openDelay: 120,
        }, {
            activator: ({ props: tooltipProps }) => vueH(VBtn, mergeProps(tooltipProps, {
                icon: params.icon,
                'aria-label': params.title,
        ...
        const createMenuButton = (params) => vueH(VMenu, {
            ...
            activator: ({ props: menuActivatorProps }) => vueH(VTooltip, {
                text: params.title,
                'aria-label': params.title,
                location: 'top',
                openDelay: 120,
            }, {
                activator: ({ props: tooltipProps }) => vueH(VBtn, mergeProps(menuActivatorProps, tooltipProps, {
        ...
        const promptForLink = () => __awaiter(this, void 0, void 0, function* () {
            ...
            const value = yield Dialogs.$prompt({
                title: currentUrl
        ...
        const promptForTableInsert = () => __awaiter(this, void 0, void 0, function* () {
            ...
            const rowsValue = yield Dialogs.$prompt({
            ...
            const colsValue = yield Dialogs.$prompt({
                title: { key: 've.editor.table.insertTitle', fallback: 'Insert Table' },
        ...
        const promptForImage = () => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const selection = getSelectionSnapshot(instance);
                const files = yield selectFile('image/*');
                const file = files === null || files === void 0 ? void 0 : files[0];
                if (!file) {
                    return;
                }
                const dataUrl = yield fileToBase64(file);
                runEditorCommand((liveEditor) => {
        ...
        watch(() => [props.label, props.placeholder, props.hint, props.errorMessage, props.required, props.readonly, props.disabled], () => {
            const instance = editor.value;
            if (!instance) {
                return;
            }
        ...
        return () => {
            ...
            const toolbar = !isReadOnly && activeToolbarItems.length > 0 ? vueH('div', {
            ...
            const editorBody = sourceMode.value
                ...
                : (instance ? [vueH(EditorContent, { editor: instance })] : []);
            return vueH('div', {
                style: {
                    width: '100%',
                    maxWidth: '100%',
                ...
                vueH(VSheet, Object.assign(Object.assign({ ref: rootEl }, attrs), { class: ['vef-tiptap', attrs.class], style: [
                        attrs.style,
                        {
                            width: '100%',
                            maxWidth: '100%',
                ...
                hasToolbarItem('fullscreen') ? vueH(VDialog, {
