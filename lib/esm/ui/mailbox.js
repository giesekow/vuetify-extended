    ...
    static loadPage(input) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                Dialogs.$error((error === null || error === void 0 ? void 0 : error.message) || { key: 've.mailbox.loadFailed', fallback: 'Failed to load mailbox items.' });
                return Mailbox.items.value;
            }
            finally {
...
Mailbox.options = ref({ pageSize: 8, title: { key: 've.mailbox.title', fallback: 'Mailbox' } });
Mailbox.items = ref([]);
...
export class MailboxView extends UIBase {
    ...
    deleteSelected() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const accepted = yield Dialogs.$confirm({
        ...
        return h(VContainer, { fluid: this.$params.fluid }, () => h(VLayout, { fullHeight: true }, () => h(VRow, { justify, align }, () => h(VCol, { cols: 12, style: { paddingTop: '16px', paddingBottom: '16px' } }, () => h('div', { style: { width: '100%', display: 'flex', justifyContent: this.$params.horizontalAlign === 'left' ? 'flex-start' : this.$params.horizontalAlign === 'right' ? 'flex-end' : 'center' } }, [
            h(VCard, { elevation: 4, style: { width, maxWidth: '100%', overflow: 'hidden' } }, () => [
                h(VCardTitle, { style: { display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' } }, () => [
                    ...
                    h(VSpacer),
                    h(VBtn, { icon: 'mdi-refresh', variant: 'text', title: this.$uiText('ve.common.refresh', 'Refresh'), 'aria-label': this.$uiText('ve.common.refresh', 'Refresh'), onClick: () => __awaiter(this, void 0, void 0, function* () { this.clearSelection(); yield Mailbox.refresh(this.$params.pageSize); }) }),
                        ...
                        : h('div', { style: { maxHeight: listMaxHeight, overflowY: 'auto' } }, [
                            h(VList, { lines: 'three' }, () => Mailbox.$items.map((item) => h(VListItem, {
                                ...
                                append: () => h('div', { style: { display: 'flex', alignItems: 'center', gap: '4px' } }, [
                                    ...
                                    h(VBtn, { icon: 'mdi-delete-outline', size: 'small', variant: 'text', title: this.$uiText('ve.common.remove', 'Remove'), 'aria-label': this.$uiText('ve.common.remove', 'Remove'), onClick: (ev) => { ev.stopPropagation(); Mailbox.remove(item); this.selectedIds.value = this.selectedIds.value.filter((id) => id !== item.id); } }),
                                ]),
                            })))
                        ]),
                    h(VDivider),
