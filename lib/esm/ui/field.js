...
import { createFieldPaginationEvent, normalizeFieldPaginationValue, updateFieldPaginationValue, } from "./widgets/field-pagination-state";
...
import { interpolateUITextTemplate, isUITextDescriptor, isUIValidationMessage, resolveUIText } from "./runtime";
import { compareExactDecimals, normalizeExactDecimal, resolveDecimalPlaces } from "../misc/decimal";
import { resolveUITableHeaders } from "./table-header";
const latexPackages = [
    'amsmath',
    'amsthm',
    'bussproofs',
...
export class Field extends UIBase {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const file of files) {
                if (maxSize > 0 && file.size > (maxSize * 1024)) {
                    Dialogs.$error(this.$uiText('ve.validation.fileMaxSize', '{file} exceeds the maximum allowed size of {max} KB.', { file: file.name, max: maxSize }));
                    continue;
                }
    ...
    openMediaItem(item) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof source === 'string' || (typeof Blob !== 'undefined' && source instanceof Blob)) {
                yield Dialogs.$previewFile(source, {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (this.params.value.type === 'code' && this.params.value.lang === 'latex') {
                try {
                    ...
                    const webtex = yield import('webtex');
                    let fulltext = value || '';
    ...
    preprocess(value) {
        ...
        if (this.params.value.type === 'decimal') {
            const values = Array.isArray(value) ? value : [value];
            const normalized = values.map((item) => normalizeExactDecimal(item, this.decimalPlaces()).value);
    ...
    autocompleteTableFallbackItem(value) {
        ...
        nestedProperty.set(item, itemTitle, String(value !== null && value !== void 0 ? value : ''));
        return item;
    }
    ...
    addAutocompleteTableItem() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const item = this.autocompleteTablePendingRawItem();
            const storedValue = this.autocompleteTableStoredValue(item);
            const appended = appendUniqueAutocompleteValue(this.autocompleteTableStoredValues(), storedValue, (left, right) => this.autocompleteValuesEqual(left, right));
            if (!appended.added) {
                return;
            }
    ...
    autocompleteTableText(value, key, fallback, values) {
        if (isUITextDescriptor(value)) {
            return resolveUIText(Object.assign(Object.assign({}, value), { values: Object.assign(Object.assign({}, (value.values || {})), (values || {})) }), fallback);
        }
        if (value !== undefined && value !== null) {
    ...
    validate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isUIValidationMessage(decimalResult))
    ...
    rules() {
        ...
        if (this.params.value.validation) {
            ...
            if (v.max)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.max.value, 'lte', 've.validation.max', 'Value cannot exceed {max}', { max: v.max.value }) : $v.max(v.max.value, v.max.converter || converter));
            if (v.min)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.min.value, 'gte', 've.validation.min', 'Value cannot be below {min}', { min: v.min.value }) : $v.min(v.min.value, v.min.converter || converter));
            if (v.gt)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.gt.value, 'gt', 've.validation.greaterThan', 'Value should be greater than {value}', { value: v.gt.value }) : $v.gt(v.gt.value, v.gt.converter || converter));
            if (v.lt)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.lt.value, 'lt', 've.validation.lessThan', 'Value should be less than {value}', { value: v.lt.value }) : $v.lt(v.lt.value, v.lt.converter || converter));
            if (v.gte)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.gte.value, 'gte', 've.validation.greaterThanOrEqual', 'Value should be greater than or equal to {value}', { value: v.gte.value }) : $v.gte(v.gte.value, v.gte.converter || converter));
            if (v.lte)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.lte.value, 'lte', 've.validation.lessThanOrEqual', 'Value should be less than or equal to {value}', { value: v.lte.value }) : $v.lte(v.lte.value, v.lte.converter || converter));
            if (v.neq)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.neq.value, 'neq', 've.validation.notEqual', 'Value cannot be equal to {value}', { value: v.neq.value }) : $v.neq(v.neq.value, v.neq.converter || converter));
            if (v.eq)
                items.push(this.params.value.type === 'decimal' ? this.decimalComparisonRule(v.eq.value, 'eq', 've.validation.equal', 'Value must be equal to {value}', { value: v.eq.value }) : $v.eq(v.eq.value, v.eq.converter || converter));
            if (v.in)
    ...
    resolveValidationRule(rule) {
        ...
        return (value) => {
            ...
            if (result && typeof result.then === 'function') {
                return result.then((resolved) => isUIValidationMessage(resolved) ? resolveUIText(resolved) : resolved);
            }
    ...
    buildAutocomplete(props, context) {
        ...
        return h(VAutocomplete, Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, this.modelBinding()), this.inputIconProps()), { autofocus: this.params.value.autofocus, label: this.resolvedLabel(), hint: this.resolvedHint(), persistentHint: !!this.params.value.hint, placeholder: this.resolvedPlaceholder(), clearable: this.params.value.clearable || false, color: this.params.value.color || "primary", variant: this.params.value.variant || ((_a = Field.defaultParams) === null || _a === void 0 ? void 0 : _a.variant), itemTitle: this.params.value.itemTitle || 'name', itemValue: Master.resolveItemValueField(this.selectItems.value, this.params.value.itemValue || this.params.value.idField), readonly: this.$readonly, items: this.selectItems.value, loading: this.autocompleteLoading.value, autoSelectFirst: true, returnObject: this.params.value.returnObject, valueComparator: (left, right) => this.autocompleteValuesEqual(left, right), multiple: this.params.value.multiple }), serverAutocompleteProps), this.popupLifecycleProps()), (this.params.value.menuTransition !== undefined ? {
    ...
    buildAutocompleteTable(props, context) {
        ...
        autocompleteSlots.append = () => h(VBtn, {
            color: this.params.value.color || 'primary',
            variant: 'tonal',
            size: 'small',
            icon: true,
        ...
        const headers = resolveUITableHeaders(this.autocompleteTableHeaders.value, (value) => this.$text(value));
        ...
        return h('div', {
            ...
            ...(!this.$readonly ? [
                h(VAutocomplete, Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ modelValue: this.autocompleteTablePendingItem.value, "onUpdate:modelValue": (value) => {
                        this.autocompleteTablePendingItem.value = value;
                    } }, this.inputIconProps()), { autofocus: this.params.value.autofocus, label: this.resolvedLabel(), hint: this.resolvedHint(), persistentHint: !!this.params.value.hint, placeholder: this.resolvedPlaceholder(), clearable: this.params.value.clearable !== false, color: this.params.value.color || 'primary', variant: this.params.value.variant || ((_a = Field.defaultParams) === null || _a === void 0 ? void 0 : _a.variant), itemTitle: this.params.value.itemTitle || 'name', itemValue: Master.resolveItemValueField(this.selectItems.value, this.params.value.itemValue || this.params.value.idField), items: this.selectItems.value, loading: this.autocompleteLoading.value || this.optionLoading.value, autoSelectFirst: true, returnObject: true, valueComparator: (left, right) => this.autocompleteValuesEqual(left, right), rules: tableRules }), serverAutocompleteProps), this.popupLifecycleProps()), (this.params.value.menuTransition !== undefined ? {
                    menuProps: Object.assign(Object.assign({}, serverAutocompleteProps.menuProps), { transition: this.params.value.menuTransition }),
    ...
    buildCollection(props, context) {
        const h = this.$h;
        const headers = resolveUITableHeaders(this.collectionHeaders, (value) => this.$text(value));
            ...
            ...(this.collectionLoaded.value ? [
                h(VCol, {
                    ...
                    onAfterEnter: () => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if ((yield finishDialogEntry((_b = this.collectionOverlay) === null || _b === void 0 ? void 0 : _b.contentEl)) &&
        ...
        if (this.isEditting && !focused) {
            if (this.params.value.type === 'decimal' && this.modelValue.value !== undefined && this.modelValue.value !== null) {
                ...
                try {
                    ...
                    const dvalue = values.map((value) => normalizeExactDecimal(value, this.decimalPlaces()).value);
