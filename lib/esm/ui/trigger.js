...
export class Trigger extends UIBase {
    ...
    buildMobileSideActions(buttons) {
        ...
        return h('div', {
            style: {
                ...
                activator: ({ props: activatorProps }) => h(VBtn, Object.assign(Object.assign({}, activatorProps), { variant: 'outlined', color: 'secondary', prependIcon: 'mdi-dots-vertical', size: 'small' }), () => 'Actions'),
                default: () => h(VCard, {
                    elevation: 2,
                    style: {
                        width: this.clampToViewport(this.params.value.sideButtonWidth, this.params.value.sideButtonWidth || 180),
    ...
    buildDefaultButtons() {
        if (this.hasRemoveAccess.value && this.selected.value && this.selected.value.length > 0) {
            return this.getAdditionalButtons().concat([
                new Button(Object.assign({ text: 'Remove', color: 'warning' }, (this.params.value.removeButton || {})), {
                    onClicked: () => this.onRemoveClicked()
                }),
                ...(this.params.value.multiple && this.selected.value && this.selected.value.length > 0 ? [
                    new Button(Object.assign({ text: this.params.value.mode === 'display' ? 'View' : 'Edit', color: 'success' }, (this.params.value.mode === 'display' ? this.params.value.viewButton || {} : this.params.value.editButton || {})), {
    ...
    onRemoveClicked() {
        return __awaiter(this, void 0, void 0, function* () {
            const confirm = yield Dialogs.$confirm('Remove selected items?');
            if (!confirm)
                return;
            const items = this.selected.value || [];
            let hasError = false;
