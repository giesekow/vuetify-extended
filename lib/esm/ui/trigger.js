...
export class Trigger extends UIBase {
            ...
            if (this.options.query) {
                ...
                if ((this.selectedSearchFields.value || []).length > 0) {
                    ...
                    if ((_a = this.searchFieldData) === null || _a === void 0 ? void 0 : _a.callback) {
                        ...
                        addedQ = this.selectedSearchFields.value.map((f) => Master.getItemId(f)).filter((fieldId) => fieldId || fieldId === 0).map((fieldId) => {
                            const fq = {};
    ...
    refreshResults(options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const refreshPromise = this.resultsRefreshPromise || (() => __awaiter(this, void 0, void 0, function* () {
    ...
    buildTitle(props, context) {
        const h = this.$h;
        return h(VCardTitle, {}, () => [
            h('span', {}, this.$text(this.$params.title))
        ]);
    }
    buildSubTitle(props, context) {
        const h = this.$h;
        return h(VCardSubtitle, {}, () => h('span', {}, this.$text(this.params.value.subtitle)));
    }
    ...
    buildFilterBar() {
        const h = this.$h;
        return [
            h(VCol, { cols: 12 }, () => h(VAutocomplete, {
                variant: 'outlined',
    ...
    buildResultTable() {
        ...
        return [
            h(VCol, {
                ...
                headers: resolveUITableHeaders(this.computedHeaders.value, (value) => this.$text(value)),
    ...
    buildDefaultButtons() {
        if (this.hasRemoveAccess.value && this.selected.value && this.selected.value.length > 0) {
            return this.getAdditionalButtons().concat([
                new Button(Object.assign({ text: this.$uiText('ve.common.remove', 'Remove'), color: 'warning' }, (this.params.value.removeButton || {})), {
                    onClicked: () => this.onRemoveClicked()
                }),
                ...(this.params.value.multiple && this.selected.value && this.selected.value.length > 0 ? [
                    new Button(Object.assign({ text: this.params.value.mode === 'display' ? this.$uiText('ve.common.view', 'View') : this.$uiText('ve.common.edit', 'Edit'), color: 'success' }, (this.params.value.mode === 'display' ? this.params.value.viewButton || {} : this.params.value.editButton || {})), {
                        onClicked: () => this.onProcessMultiple()
                    })
                ] : []),
                new Button(Object.assign({ text: this.$uiText('ve.common.cancel', 'Cancel'), color: 'secondary' }, (this.params.value.cancelButton || {})), {
    ...
    onRemoveClicked() {
        return __awaiter(this, void 0, void 0, function* () {
            const confirm = yield Dialogs.$confirm({ key: 've.trigger.confirmRemoveSelected', fallback: 'Remove selected items?' });
            if (!confirm)
                return;
            const items = this.selected.value || [];
            let hasError = false;
    ...
    setActiveRowByItem(item) {
        ...
        const items = this.items.value || [];
        const itemId = Master.getItemId(item, this.params.value.idField);
        const index = items.findIndex((entry) => Master.matchesItemId(entry, itemId, this.params.value.idField) || entry === item);
        if (index >= 0) {
            this.activeRowIndex.value = index;
            this.scrollActiveRowIntoView();
        }
...
export const $TG = (params, options) => new Trigger(params || {}, options || {});
