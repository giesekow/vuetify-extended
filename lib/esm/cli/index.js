...
import { runBootstrapAppCommand } from './bootstrap-app';
import { runCreateCollectionCommand, runCreateFormCommand, runCreateMenuItemCommand, runCreateReportCommand, runCreateSubMenuCommand, runCreateTriggerCommand, } from './create-ui';
import { runCreateDashboardCommand as runCreateDashboardCommandAdvanced, runCreateDashboardWidgetCommand as runCreateDashboardWidgetCommandAdvanced, runCreateFieldCommand as runCreateFieldCommandAdvanced, runCreatePageCommand as runCreatePageCommandAdvanced, runCreatePartCommand as runCreatePartCommandAdvanced, runCreateServiceCommand as runCreateServiceCommandAdvanced, runCreateValidatorCommand as runCreateValidatorCommandAdvanced, runDoctorCommand as runDoctorCommandAdvanced, runMigrateCommand as runMigrateCommandAdvanced, } from './advanced-ui';
import { runBootstrapThemeCommand, runCreateAssetServiceCommand, runCreateAutocompleteSourceCommand, runCreateDashboardDataSourceCommand, runCreateDialogFormCommand, runCreateHeaderItemCommand, runCreateReportActionCommand, runCreateRouteCommand, runCreateTriggerActionCommand, runCreateTriggerFieldCommand, } from './productivity-ui';
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (command === 'bootstrap' && subcommand === 'app') {
            ...
            const exitCode = yield runBootstrapAppCommand({
                cwd: process.cwd(),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'report') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateReportCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'route') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateRouteCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dialog-form') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateDialogFormCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'page') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreatePageCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'form') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateFormCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'field') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateFieldCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger-field') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateTriggerFieldCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger-action') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateTriggerActionCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'report-action') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateReportActionCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'part') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreatePartCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateTriggerCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateDashboardCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard-widget') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateDashboardWidgetCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard-data-source') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateDashboardDataSourceCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'collection') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateCollectionCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'service') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateServiceCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'asset-service') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateAssetServiceCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'autocomplete-source') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateAutocompleteSourceCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'validator') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateValidatorCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'menu-item') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateMenuItemCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'header-item') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateHeaderItemCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && (subcommand === 'sub-menu' || subcommand === 'submenu')) {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runCreateSubMenuCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'doctor') {
            ...
            process.exitCode = yield runDoctorCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'bootstrap' && subcommand === 'theme') {
            const parsed = parseNamedArgs(args.slice(2));
            process.exitCode = yield runBootstrapThemeCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'migrate') {
            const parsed = parseNamedArgs(args.slice(1));
            process.exitCode = yield runMigrateCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
