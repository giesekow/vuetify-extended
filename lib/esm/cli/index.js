...
export function main(args = process.argv.slice(2)) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'report') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateReportCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'route') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateRouteCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dialog-form') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateDialogFormCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'page') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreatePageCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'form') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateFormCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'field') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateFieldCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger-field') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateTriggerFieldCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger-action') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateTriggerActionCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'report-action') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateReportActionCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'part') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreatePartCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'trigger') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateTriggerCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateDashboardCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard-widget') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateDashboardWidgetCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'dashboard-data-source') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateDashboardDataSourceCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'collection') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateCollectionCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'service') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateServiceCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'asset-service') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateAssetServiceCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'autocomplete-source') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateAutocompleteSourceCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'validator') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateValidatorCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'menu-item') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateMenuItemCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && subcommand === 'header-item') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateHeaderItemCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if ((command === 'create' || command === 'make') && (subcommand === 'sub-menu' || subcommand === 'submenu')) {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runCreateSubMenuCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'doctor') {
            ...
            return yield runDoctorCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'bootstrap' && subcommand === 'theme') {
            const parsed = parseNamedArgs(args.slice(2));
            return yield runBootstrapThemeCommand({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
        ...
        if (command === 'migrate') {
            const parsed = parseNamedArgs(args.slice(1));
            return yield runMigrateCommandAdvanced({
                cwd: process.cwd(),
                dryRun: hasFlag(parsed, 'dry-run'),
                force: hasFlag(parsed, 'force'),
                interactive: resolveInteractiveMode(parsed),
