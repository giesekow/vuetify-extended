...
import { MailboxBell, MailboxView } from '../ui/mailbox';
import { Menu, MenuItem } from '../ui/menu';
import { Part } from '../ui/part';
import { Report } from '../ui/report';
import { Selector } from '../ui/selector';
import { AppTitleBlock, EnvironmentTag, StatusBadge, UserArea } from '../ui/shell';
import { Trigger } from '../ui/trigger';
import { Master } from '../master';
import { setVuetifyExtendedI18n } from '../ui/runtime';
...
export function configureVuetifyExtendedDefaults(defaults, reset) {
    ...
    if (defaults.appTitleBlock)
        AppTitleBlock.setDefault(defaults.appTitleBlock, reset);
    if (defaults.button)
        Button.setDefault(defaults.button, reset);
    if (defaults.collection)
        Collection.setDefault(defaults.collection, reset);
    if (defaults.confirm)
        Dialogs.setConfirmDefault(defaults.confirm, reset);
    if (defaults.dialogForm)
        DialogForm.setDefault(defaults.dialogForm, reset);
    if (defaults.documentPreview)
        Dialogs.setDocumentPreviewDefault(defaults.documentPreview, reset);
    if (defaults.environmentTag)
        EnvironmentTag.setDefault(defaults.environmentTag, reset);
    if (defaults.field)
        Field.setDefault(defaults.field, reset);
    if (defaults.filePreview)
        Dialogs.setFilePreviewDefault(defaults.filePreview, reset);
    if (defaults.form)
        Form.setDefault(defaults.form, reset);
    if (defaults.iframe)
        Dialogs.setIframeDefault(defaults.iframe, reset);
    if (defaults.imagePreview)
        Dialogs.setImagePreviewDefault(defaults.imagePreview, reset);
    if (defaults.info)
        Dialogs.setInfoDefault(defaults.info, reset);
    if (defaults.mailboxBell)
        MailboxBell.setDefault(defaults.mailboxBell, reset);
    if (defaults.mailboxView)
        MailboxView.setDefault(defaults.mailboxView, reset);
    if (defaults.master)
        Master.setDefault(defaults.master, reset);
    if (defaults.menu)
    ...
    if (defaults.notifications)
        Notifications.setDefault(defaults.notifications, reset);
    if (defaults.part)
        Part.setDefault(defaults.part, reset);
    if (defaults.prompt)
        Dialogs.setPromptDefault(defaults.prompt, reset);
    if (defaults.report)
    ...
    if (defaults.statusBadge)
        StatusBadge.setDefault(defaults.statusBadge, reset);
    if (defaults.trigger)
        Trigger.setDefault(defaults.trigger, reset);
    if (defaults.userArea)
        UserArea.setDefault(defaults.userArea, reset);
}
...
export function createVuetifyExtendedApp(options = {}) {
    ...
    AppManager.init();
    const appMain = createAppMain(options.app, Object.assign(Object.assign(Object.assign(Object.assign({}, (options.navigation ? { navigation: options.navigation } : {})), (options.menu ? { menu: options.menu } : {})), (options.udfs ? { udfs: options.udfs } : {})), (options.makeUDF ? { makeUDF: options.makeUDF } : {})));
