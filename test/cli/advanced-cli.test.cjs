...
async function testBootstrapAppScaffoldsHomeStartup() {
...
`);
  ...
  const result = await runCli(cwd, ['bootstrap', 'app', '--non-interactive', '--backend', 'none']);
...
async function main() {
  await testBootstrapAppScaffoldsHomeStartup();

  const cwd = makeTempDir('ve-cli-advanced-');

  let result = await runCli(cwd, ['create', '--help']);
  ...
  result = await runCli(cwd, ['create', 'autocomplete-source', '--help']);
...
`);
  ...
  result = await runCli(cwd, ['create', 'page', 'report', 'people', '-y', '--title', 'People Workspace', '--object-type', 'people']);
  ...
  result = await runCli(cwd, ['create', 'field', 'people', '--non-interactive', '--label', 'Email', '--storage', 'email', '--cols', '6', '--required', 'true']);
  ...
  result = await runCli(cwd, ['create', 'part', 'people', '--non-interactive', '--title', 'More Details']);
  ...
  result = await runCli(cwd, ['create', 'dashboard', 'ops', '--non-interactive', '--title', 'Ops Dashboard']);
  ...
  result = await runCli(cwd, ['create', 'page', 'trigger', 'finder', '-y', '--title', 'Finder Trigger', '--object-type', 'people']);
  ...
  result = await runCli(cwd, ['create', 'dashboard-widget', 'ops', '--non-interactive', '--type', 'chart', '--title', 'Revenue Trend']);
  ...
  result = await runCli(cwd, ['create', 'route', 'people-route', '--non-interactive', '--page', 'people', '--target', 'report', '--path', '/people', '--title', 'People Route']);
  ...
  result = await runCli(cwd, ['create', 'dialog-form', 'quick-edit', '--non-interactive', '--title', 'Quick Edit']);
  ...
  result = await runCli(cwd, ['create', 'trigger-field', 'finder', '--non-interactive', '--label', 'Status', '--storage', 'status', '--type', 'select', '--cols', '6']);
  ...
  result = await runCli(cwd, ['create', 'trigger-action', 'finder', '--non-interactive', '--text', 'Bulk Approve', '--icon', 'mdi-check', '--color', 'success']);
  ...
  result = await runCli(cwd, ['create', 'report-action', 'people', '--non-interactive', '--text', 'Summary', '--icon', 'mdi-information-outline', '--color', 'primary']);
  ...
  result = await runCli(cwd, ['create', 'dashboard-data-source', 'ops', '--non-interactive', '--name', 'dashboard-data']);
  ...
  result = await runCli(cwd, ['create', 'asset-service', 'assets', '--non-interactive', '--path', 'assets']);
  ...
  result = await runCli(cwd, ['create', 'autocomplete-source', 'people-autocomplete', '--non-interactive', '--service-path', 'people', '--label-field', 'name', '--value-field', '_id']);
...
`);
  ...
  result = await runCli(cwd, ['create', 'header-item', '--non-interactive', '--kind', 'action', '--region', 'end', '--text', 'Open Help', '--icon', 'mdi-help-circle-outline', '--color', 'secondary']);
  ...
  result = await runCli(cwd, ['bootstrap', 'theme', '--non-interactive', '--mode', 'dark']);
  ...
  result = await runCli(cwd, ['create', 'service', 'users', '--non-interactive']);
  ...
  result = await runCli(cwd, ['create', 'validator', 'order-code', '--non-interactive']);
  assertSuccess(result, 'create validator');
  assert.ok(fs.existsSync(path.join(cwd, 'src/validators/order-code.ts')));

  result = await runCli(cwd, ['add', 'menu-item', 'ops', '--non-interactive', '--target', 'dashboard', '--text', 'Ops Dashboard']);
  ...
  result = await runCli(cwd, ['create', 'sub-menu', 'settings', '--non-interactive', '--title', 'Settings', '--text', 'Settings Hub', '--sub-text', 'Open workspace settings tools.', '--icon', 'mdi-cog-outline', '--color', 'secondary']);
  ...
  result = await runCli(cwd, ['create', 'menu-item', 'people', '--non-interactive', '--target', 'report', '--text', 'People Report', '--menu-file', 'src/menu/settings.ts']);
...
`);
  ...
  result = await runCli(cwd, ['migrate', '--apply']);
  ...
  result = await runCli(cwd, ['doctor']);
