...
function paginationButtonVariant(field: Field) {
...
export function buildPaginationWidget(field: Field): VNode {
  const h = field.$h;
  ...
  const activeVariant = paginationButtonVariant(field) as any;

  const pageSizeControls = params.showItemsPerPage === false
    ? []
    : [
        h('div', { class: ['vef-pagination__page-sizes'] }, [
          h('span', { class: ['vef-pagination__label'] }, field.$text(
            params.label,
            field.$uiText('ve.field.pagination.itemsPerPage', 'Items per page'),
          )),
  ...
  const statusControls = h('div', {
    ...
    } as any, () => field.$uiText('ve.common.prev', 'Previous')),
    ...(params.showPageInfo === false ? [] : [
      h('span', { class: ['vef-pagination__page-info'] }, field.$uiText(
        've.field.pagination.pageOf',
        'Page {page} of {total}',
        { page: value.page, total: pageCount },
      )),
    ]),
    h(VBtn, {
      size: 'small',
    ...
    } as any, () => field.$uiText('ve.common.next', 'Next')),
  ]);
